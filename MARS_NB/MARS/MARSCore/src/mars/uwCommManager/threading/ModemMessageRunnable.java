...
/**
 * The new runnable is to be used with the Executer class from
 * java.util.concurrent There should be one instance of this for each AUV with a
 * modem
 *
 * @version 0.3
...
public class ModemMessageRunnable implements Runnable {
    ...
    /**
     * The distanceTriggers that are added to all CommuncationMessages that are
     * moved to computeMessages
     */
     ...
     *
     * @since 0.1
     * @param modem_bandwidth the maximum bandwidth the modem has in kilobyte
     * per secound
    ...
    /**
     * @since 0.3 All non-trivial initialization
                ...
                if (e.getKey().equals(CommOptionsConstants.OPTIONS_SPEED_OF_SOUND_METHOD)) {
                    ...
                    if (Boolean.parseBoolean(e.getNewValue())) {
                        for (ANoiseByDistanceGenerator a : noiseGenerators) {
                            if (a.getName().equals(NoiseNameConstants.GAUSSIAN_WHITE_NOISE)) {
                                return;
                            }
                        }
                        noiseGenerators.add(new AdditiveGaussianWhiteNoise(1, 1 / 4f, shippingFactor, windspeed));
    ...
    /**
     * Determine the corresponding function to the optionsValue and calculate
     * the current speed of sound with the right method
     *
     * @param optionsValue The value of the selected Item in the
     * NoiseOptionsPanel
    ...
    protected void determineSpeedOfSoundMethod(int optionsValue, PhysicalEnvironment env) {
        ...
        switch (optionsValue) {
            case 1:
                speedOfSound = getUnderWaterSoundSpeedLubberGraaffB(env.getFluid_temp());
                break;
            case 2:
                speedOfSound = getUnderWaterSoundSpeedMarczak(env.getFluid_temp());
                break;
            case 3:
                speedOfSound = getUnderWaterSoundSpeedCoppens(env.getFluid_temp(), env.getFluid_salinity(), depth);
                break;
            case 4:
                speedOfSound = getUnderWaterSoundSpeedMackenzie(env.getFluid_temp(), env.getFluid_salinity(), depth);
                break;
            case 5:
                speedOfSound = getUnderWaterSoundSpeedDelGrosso(env.getFluid_temp(), env.getFluid_salinity(), depth);
                break;
            case 6:
                speedOfSound = getUnderWaterSoundSpeedChenMillero(env.getFluid_temp(), env.getFluid_salinity(), depth);
                break;
            default:
                speedOfSound = getUnderWaterSoundSpeedLubberGraaffA(env.getFluid_temp());
                break;
        }
    ...
    /**
     * three steps: transform all new Communcation messages to
     * CommunicationDataChunks take the next pending DataChunk and add it to the
     * sent chunks make a tick with all sent chunks
     *
     * @since 0.1
     */
    @Override
    ...
    /**
     * Make a step with all sent chunks kill all chunks that have reached
     * maximum range
     *
     * @since 0.1
     *
    ...
    /**
     *
     * take all new enqueued CommunicationMessages and converts them into
     * CommuncationDataChunks
     *
     * @since 0.1
     */
    ...
    public synchronized void removeANoiseGeneratorByName(String name) {
        ...
        if (toBeRemoved != null) {
            noiseGenerators.remove(toBeRemoved);
        }
    }
