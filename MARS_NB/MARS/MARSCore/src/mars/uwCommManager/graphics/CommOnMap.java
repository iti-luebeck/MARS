...
/**
 * This class is used for all visual effects generated by communications on the
 * minimap.
 *
 * @version 0.2
 * @author Jasper Schwinghammer
 */
public class CommOnMap implements TriggerEventListener {
    ...
    List<TriggerOutOfRangeEvent> triggerOOREventQueue;
    ...
    public CommOnMap(boolean active, boolean borders, boolean showLinks) {
        ...
        traceHitEventQueue = new LinkedList();
        traceBlockedEventQueue = new LinkedList();
        triggerOOREventQueue = new LinkedList();
    }
    ...
    public boolean init(MapState mapState, AUV_Manager auvManager, MARS_Settings marsSettings, AssetManager assetManager, MARS_Main app) {
        if (mapState == null || auvManager == null || marsSettings == null) {
            return false;
        }
    ...
    /**
     * Add propagation distance features to AUVs code moved from
     * mars.states.MapState
     *
     * @since 0.2.1
        ...
        app.enqueue(new Callable<Object>() {
            ...
            public Object call() {
                ...
                while (it.hasNext()) {
                    ...
                    uw_geom_border_mat.setColor("Color", ColorRGBA.White);
...
//                                if (i.getFloorBounces() == 0 || i.getSurfaceBounces() == 0) {
//                                    String connectionName = auv.getName() + "->" + i.getAUVName();
//                                    if (auvNodes.get(i.getAUVName()) == null) {
//                                        return;
//                                    }
...
//                }
        ...
        for (TraceHitAUVEvent e : copyList) {
            ...
            if (distanceNode == null) {
                ...
                if (auvNodes.get(e.getTargetAUVName()) == null) {
                    return;
                }
                String connectionName = e.getSourceAUVName() + "->" + e.getTargetAUVName();
                Geometry uwgeom = (Geometry) distanceNode.getChild(connectionName);
                if (uwgeom == null) {
                    attachLine(connectionName, distanceNode, new Vector3f(0, 0, 0), auvNodes.get(e.getTargetAUVName()).getWorldTranslation().subtract(node.getWorldTranslation()));
                } else {
                    inactivePaths.remove(uwgeom);
                    Line line = (Line) uwgeom.getMesh();
                    line.updatePoints(new Vector3f(0, 0, 0), auvNodes.get(e.getTargetAUVName()).getWorldTranslation().subtract(node.getWorldTranslation()));
                    uwgeom.setCullHint(Spatial.CullHint.Inherit);
                }
            }
        }
        ...
        for (TraceBlockedEvent e : blockedTraceQueueCopy) {
            if (e.getTraces().size() == 2) {
                ...
                if (distanceNode.getChild(connectionName) == null) {
                    return;
                }
                distanceNode.getChild(connectionName).setCullHint(Spatial.CullHint.Always);
            }
        }
        List<TriggerOutOfRangeEvent> outOfRangeCopy;
        synchronized (this) {
            outOfRangeCopy = new LinkedList(triggerOOREventQueue);
            triggerOOREventQueue.clear();
        }
        for (TriggerOutOfRangeEvent e : outOfRangeCopy) {
            String distanceNodeName = e.getSourceAUVName() + "distances";
            String connectionName = e.getSourceAUVName() + "->" + e.getTargetAUVName();
            Node node = (Node) auvNodes.get(e.getSourceAUVName());
            Node distanceNode = (Node) node.getChild(distanceNodeName);
    ...
    public void triggerEventHappened(ATriggerEvent e) {
        synchronized (this) {
            if (e.getEventID() == CommunicationEventConstants.TRACE_HIT_AUV_EVENT) {
                traceHitEventQueue.add((TraceHitAUVEvent) e);
